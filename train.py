...
def main(cfg):
    ...
    anydepth = cfg['model_config']['anydepth']  #for databinary it is not used  
    ...
    if model_type == 'single':
        ...
        config_vit.n_skip = 0
        #config_vit.patches.grid = (int(input_size[0] / 16), int(input_size[1] / 16))

        model = ViT_seg(config_vit, img_size=input_size, num_classes=cfg['model_config']['num_class']).cuda()
        #model.load_from(weights=np.load("/kuacc/users/dozkan23/hpc_run/TransUnet/TransUnet/ViT-B_16.npz"))
