...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.5:   #sırayı degistirtdim, augmentation önceydi orjinalde.
...
class Data_Binary(Dataset):
    ...
    def transform(self,sample):
        ...
        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        #image = self.normalizeTorch(image.astype(np.float32)).unsqueeze(0)
        #label = torch.from_numpy(label.astype(np.float32)).unsqueeze(0)  #if binary 
        label = torch.from_numpy(label.astype(np.float32))  #if multiclass
    ...
    def __getitem__(self, idx):
        print("Get item worked")
        img_path=self.image_list[idx]
        image=cv2.imread(img_path, 0)
        label_path=img_path[:img_path.rfind(".")]+".png"
        if "test" in label_path:
            label_path=label_path.replace("test", "golds")
        elif "train" in label_path:
            label_path=label_path.replace("train", "golds")
        else:
            label_path=label_path.replace("val", "golds")
